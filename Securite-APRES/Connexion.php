...
<body>
  <div class="container" id="MainConnexion">
    <form name='f' id='f' action="#" method="post">
      <input type="text"      name="user"   class="form-control" placeholder="Nom d'utilisateur" maxlength="50"/>
      <div id=espace></div>
      <input type="password"  name="mdp"    class="form-control" placeholder="Mot de passe" maxlength="50"  autocomplete="off"/>
      <div id=espace></div>
...
if (!empty($_POST)) //On teste si l'utilisateur a envoyé le formulaire
{
  ...
  else //Aucun des champs n'est vide, on peut contacter la BDD
  {
    ...
    if($query->execute(array('pseudo' => $_POST["user"]))) // On vérifie qu'il n'y a pas d'erreur dans la commande
    {
      if($query->rowCount() > 0)
      {
        while ($data = $query->fetch())
        {
          if (password_verify($_POST["mdp"], $data['pass'])) //Vérifie si le mdp hashé dans la bdd correspond
          {
